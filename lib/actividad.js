...
import { sumarMensajeDiario, getRacha, setRacha, ganarCoins, mensajesPorHora, mensajesPorDia, podarActividadHoraria } from "../database-functions.js";
...
export function textoActividad(chat, ahora = new Date()) {
  ...
  const lineas = [`Hoy: *${porDia.get(dias[0]) || 0}* mensajes · ayer ${porDia.get(dias[1]) || 0}`, `Últimos 7 días: *${total}* · ${Math.round(total / 7)} por día`, ""];
