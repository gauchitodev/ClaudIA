// Couples, requests and marriages. A couple is a row in the parejas table; an unanswered request is a row in
// solicitudes_pareja and expires on its own; an ended relationship lands in exparejas. All the logic goes through
// here and returns { ok, motivo }: the plugins only pick the wording. People are identified by lid, as in mentions.
...
import { parentescoDe } from "./familia.js";
import { lidDeDigitos } from "./menciones.js";

export const PAREJAS = {
  DIAS_SOLICITUD: 7, // an unanswered request expires after a week
  DIAS_PARA_CASARSE: 7,
  ENOJO_MS: 60 * 60 * 1000, // how long the huff over an attempted infidelity lasts
  ABANDONO_DIAS: 3, // days without a kiss or a gift before the next kiss can be turned down
  CHANCE_RECHAZO_ABANDONO: 0.5,
};

// Injectable for the tests.
export const _dep = { azar: Math.random };

export const parejaDe = (lid) => parejaEnBase(lid);
export const sonPareja = (a, b) => parejaDe(a)?.pareja === b;

// the pending request this person made, if it hasn't expired
...
export function pedirPareja(de, para, chat = "", ahora = Date.now()) {
  ...
  const parentesco = parentescoDe(de, para);
  if (parentesco) return { ok: false, motivo: "parientes", parentesco };
...
// .aceptar @x: x must have asked, and neither of them can be in another couple
export function aceptarPareja(quien, de, ahora = Date.now()) {
  if (quien === de) return { ok: false, motivo: "mismo" };
  const parentesco = parentescoDe(quien, de);
  if (parentesco) return { ok: false, motivo: "parientes", parentesco };
...
// .terminar: the relationship moves to both of their exes
...
// a person's exes, not counting their current partner if they got back together
export function exParejasDe(lid) {
  const actual = parejaDe(lid)?.pareja;
  return exEnBase(lid).filter((x) => x !== actual);
}

// .casarse: only with a partner of at least a week
...
// .si / .no to a partner's marriage proposal
...
// For the owner: forms the couple outright, ending any they had with other people.
...
// ---- kisses, huffs and making up ----

// Someone in a couple tried to kiss another person: their partner is in a huff for a while. Returns the partner's lid.
export function enojarPareja(ofensor, por, ahora = Date.now()) {
  const p = parejaDe(ofensor);
  if (!p) return null;
  actualizarPareja(p.id, { enojada: p.pareja, enojo_hasta: ahora + PAREJAS.ENOJO_MS, enojo_por: por });
  return p.pareja;
}

// The live huff of `lid`'s partner against `lid`, or null.
export function enojoVigente(lid, ahora = Date.now()) {
  const p = parejaDe(lid);
  if (!p || p.enojada !== p.pareja || p.enojoHasta <= ahora) return null;
  return { por: p.enojoPor, hasta: p.enojoHasta, minutos: Math.max(1, Math.ceil((p.enojoHasta - ahora) / 60000)) };
}

// A gift to your partner: it gets them over a huff, and counts as attention (like a kiss). Returns true if there was a huff.
export function contentarPareja(de, para, ahora = Date.now()) {
  const p = parejaDe(de);
  if (!p || p.pareja !== para) return false;
  const habiaEnojo = p.enojada === para && p.enojoHasta > ahora;
  actualizarPareja(p.id, { enojada: "", enojo_hasta: 0, enojo_por: "", ultimo_beso: ahora });
  return habiaEnojo;
}

// A kiss between partners. Returns null if they aren't a couple; otherwise { tipo: "enojo" | "abandono" | "ok", ... }.
// With a live huff it is always turned down; after days with no kiss or gift, the first one may be turned down once.
export function besoEntrePareja(de, para, ahora = Date.now()) {
  const p = parejaDe(de);
  if (!p || p.pareja !== para) return null;
  const enojo = enojoVigente(de, ahora);
  if (enojo) return { tipo: "enojo", ...enojo };
  const ultimo = p.ultimoBeso || p.desde;
  const dias = Math.floor((ahora - ultimo) / DIA_MS);
  actualizarPareja(p.id, { ultimo_beso: ahora });
  if (dias >= PAREJAS.ABANDONO_DIAS && _dep.azar() < PAREJAS.CHANCE_RECHAZO_ABANDONO) return { tipo: "abandono", dias };
  return { tipo: "ok", casados: p.casadosDesde > 0 };
}

// ---- helpers for the owner commands ----
...
// Two people by mention, by typed "@number" or by two phone numbers with +. Returns [lid, lid] or null.
export function dosPersonas(m, text) {
  ...
  if (lids.length < 2) lids = [...texto.matchAll(/@(\d{3,})/g)].map((x) => lidDeDigitos(x[1]));
...
// ".setpareja @a @b | 3 días": how long they have been together. Without "|" it is one second; with a time it can't
// parse, null.
