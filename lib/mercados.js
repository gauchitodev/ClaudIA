// Mercados de apuestas sobre eventos reales (fútbol, clima, lo que sea): un admin abre el mercado con sus opciones
// y una hora de cierre, la gente apuesta UruCoins, y cuando el evento pasó un admin carga el resultado.
// El pago es "a pozo": todo lo apostado se reparte entre los que acertaron, proporcional a lo que puso cada uno.
// La banca no arriesga nada, solo redistribuye. Si nadie acertó, o el mercado se anula, se devuelve todo.
...
export function parsearCierre(texto, ahora = Date.now()) {
...
// ---------- crear ----------
// texto: "Título | opción 1 | opción 2 [| opción 3...] | cierre"
export function crearMercadoDesdeTexto(chat, usuario, texto) {
  ...
  const cierraEn = parsearCierre(cierreTexto);
  ...
  // cierre automático de las apuestas, y anulación con devolución si nadie carga el resultado en una semana
...
export function jugar(chat, usuario, id, textoOpcion, cantidad) {
  ...
  if (yaPuesto + cantidad > COINS.MERCADO_APUESTA_MAX) return { ok: false, error: `Máximo ${COINS.MERCADO_APUESTA_MAX} UruCoins por persona en cada mercado (ya tenés ${yaPuesto}).` };
...
export function resolver(chat, usuario, id, textoOpcion, { esOwner = false } = {}) {
  ...
  for (const a of apuestas) {
    if (a.opcion === ganadora) {
      // reparto proporcional: lo que puso sobre el pozo ganador, por el pozo total (redondeado hacia abajo)
...
// Lo llama el procesador de pendientes a la hora de cierre. Devuelve el anuncio, o null si ya no corresponde.
...
// Lo llama el procesador de pendientes una semana después del cierre: si nadie cargó el resultado, devuelve todo.
