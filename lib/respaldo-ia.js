...
const PROVEEDORES = [
  {
    ...
    // 2026-08-16; these are the replacements their docs recommend (checked 2026-09-23 in the docs: listing the models
    // takes a key).
    nombre: "groq",
  ...
  },
  {
    ...
    // The free list rotates often: check it without a key at https://openrouter.ai/api/v1/models (checked 2026-09-23).
    nombre: "openrouter",
  ...
  },
  {
    ...
    // The catalogue can be checked without a key at https://integrate.api.nvidia.com/v1/models (checked 2026-09-23).
    nombre: "nvidia",
  ...
  },
  {
    ...
    // Its models can be checked without a key at https://api.cerebras.ai/public/v1/models: gemma-4-31b is gone from
    // it, and qwen-3.8-27b took its place (checked 2026-09-23).
    nombre: "cerebras",
...
// The reply without the reasoning a model may have left in it. Qwen on Groq, by default, starts its reply with its
// thinking between <think> tags (except in JSON mode), and Cerebras doesn't document what its Qwen does: that thinking
// would reach the group as part of Claudia's answer. "" when nothing but thinking is left, which is what a thought
// cut off by max_tokens looks like.
export function sinRazonamiento(texto) {
  if (typeof texto !== "string") return "";
  const limpio = texto.replace(/<think>[\s\S]*?<\/think>/gi, "").trim();
  return /^<think>/i.test(limpio) ? "" : limpio;
}
...
async function llamarModelo(prov, modelo, texto, schema, usarJsonMode) {
  ...
  try {
    ...
    let respuesta = sinRazonamiento(data?.choices?.[0]?.message?.content);
    if (!respuesta) return { ok: false, sinCuota: false }; // no reply, or only thinking: the next model gets a go
