...
const PROVEEDORES = [
  {
    // Groq: nivel gratis sin tarjeta, limitado solo por pedidos por minuto. Los llama 3.x se retiraron el 16/8/2026;
    // estos son los reemplazos que recomienda su documentación (verificado el 6/9/2026).
    nombre: "groq",
  ...
  },
  {
    ...
    // La lista gratis rota seguido: se chequea sin clave en https://openrouter.ai/api/v1/models (verificado el 6/9/2026).
    nombre: "openrouter",
  ...
  },
  {
    ...
    // El catálogo se chequea sin clave en https://integrate.api.nvidia.com/v1/models (verificado el 6/9/2026).
    nombre: "nvidia",
...
async function llamarModelo(prov, modelo, texto, schema, usarJsonMode) {
  ...
  try {
    ...
    // El id puede venir con prefijo de proveedor ("openai/gpt-oss-20b"), así que se mira el nombre, no el principio.
