...
// Mines boards, duels and fights): each one charges up front and pays when it settles, so a restart in
...
export function devolverLoQueEstaEnJuego() {
  let apuestas = 0;
  let monedas = 0;
  for (const devolver of [devolverMesasRuleta, devolverCarreras, devolverManosBlackjack, devolverPartidasMines, devolverDuelosYPeleas]) {
    try {
      const r = devolver();
      apuestas += r.apuestas;
      monedas += r.monedas;
