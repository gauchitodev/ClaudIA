...
// The messages the bot has reacted to (🔥 streaks, 🪙 today's question, ✅ trivia, the random reactions…). WhatsApp keeps
// one reaction per person and message, so a reaction from Claudia's initiative there would replace the system's one:
// lib/vistazos.js checks here first. In memory, capped.
const reaccionadosPorElBot = new Map();
const MAX_REACCIONADOS = 1000;
function recordarReaccionDelBot(id) {
  if (!id) return;
  reaccionadosPorElBot.set(id, Date.now());
  if (reaccionadosPorElBot.size > MAX_REACCIONADOS) reaccionadosPorElBot.delete(reaccionadosPorElBot.keys().next().value);
}
export const yaReaccionoElBot = (id) => Boolean(id) && reaccionadosPorElBot.has(id);
...
export function envolverEnvios(client) {
  ...
  const envuelto = async (jid, contenido, ...resto) => {
    ...
    // look failed.
    if (enviado) {
      if (tipo === "reaccion") recordarReaccionDelBot(contenido.react?.key?.id);
      try {
