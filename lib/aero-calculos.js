// Aeronautical calculations with no API at all: crosswind and solar events (sunrise, sunset and civil twilight).
const rad = (g) => (g * Math.PI) / 180;
const deg = (r) => (r * 180) / Math.PI;

// ---------- viento cruzado ----------
// runway: "06", "6", "060", "24L" → heading in degrees. Wind: direction 0-360 and speed in kt, gusts optional.
...
function componentes(rumbo, dir, vel) {
  const angulo = ((dir - rumbo + 540) % 360) - 180; // -180..180: positive = wind from the right
...
// ---------- sol ----------
// Equations from NOAA's solar calculator. Returns the instants (UTC ms) of sunrise, sunset and civil twilight for a
// civil date (year, month, day) at a point; null where the event doesn't happen (polar day or night).
...
// UTC minutes since midnight of the event; "polar" means continuous day or night
...
// ---------- reciprocal heading ----------
// The opposite of a heading, in 1..360 (0 is written as 360).
export const reciproco = (rumbo) => ((rumbo + 180 - 1) % 360) + 1;
const grados = (r) => `${String(r).padStart(3, "0")}°`;

// "045", "45", "360", "06", "24L" → { tipo: "rumbo" | "pista", ... } or null. One or two digits up to 36 is a runway;
// three digits is a heading (018 = 18°).
...
// In a level turn n = 1 / cos(bank angle); the stall speed is multiplied by √n.
export const factorDeCarga = (angulo) => 1 / Math.cos(rad(angulo));
const coma = (n, d = 2) => n.toFixed(d).replace(".", ",");
const LIMITE_NORMAL_G = 3.8; // normal category (FAR 23)
const redondeado = (n) => Math.round(n * 100) / 100; // cos(60°) gives 1.9999999999999996 in floating point

// ".factorcarga 45", ".factorcarga 45 50" (with a 50 kt stall speed); with nothing it shows the table
