// Mines: una grilla de 5x5 con N minas. Destapás casillas de a una: cada casilla segura sube el multiplicador y podés
// retirar cuando quieras; si tocás una mina perdés la apuesta. El multiplicador es el justo por probabilidad con un
// 3 % para la banca, y el premio tiene techo de MAX_MULT veces la apuesta (sin techo, un tablero casi entero pagaría
// millones). Una partida por persona, en memoria; sin decidir en SEGUNDOS_DECISION se retira sola.
...
// Multiplicador tras k casillas seguras con m minas: la inversa de la probabilidad de acertarlas, con el margen de
// la banca y el techo.
...
// "B3", "b3", "3B" o un número del 1 al 25 → índice 0..24, o null
...
// Retira: paga apuesta por multiplicador. Sin casillas abiertas devuelve la apuesta tal cual.
...
export function destapar(chat, usuario, casillaTexto) {
  ...
  if (partida.minas.has(i)) {
    ...
    const devuelto = protegerApuesta(chat, usuario, partida.apuesta);
  ...
  const m = partida.minas.size;
  // sin casillas seguras por destapar, o con el multiplicador en el techo, no hay nada más que ganar: se retira solo
