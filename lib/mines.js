...
// For shutdown (lib/cierre.js): refunds the stake of every open game, as retiring before uncovering anything does.
// Returns { apuestas, monedas }.
export function devolverPartidasMines() {
  let apuestas = 0;
  let monedas = 0;
  for (const [k, partida] of globalThis.partidasMines) {
    clearTimeout(partida.timeout);
    globalThis.partidasMines.delete(k);
    ganarCoins(partida.chat, partida.usuario, partida.apuesta, "casino_mines_devolucion");
    apuestas++;
    monedas += partida.apuesta;
  }
