...
function cobrarSegundaApuesta(partida, mano, accion) {
  ...
  const cobro = cobrarApuesta(partida.chat, partida.usuario, mano.cantidad, "casino_blackjack");
  if (!cobro.ok) {
...
export function seguro(chat, usuario) {
  ...
  if (!partida.ofreceSeguro) {
    ...
    // Con un as a la vista el motivo real es que la mano ya arrancó: decir "cuando la banca muestra un as" contradecía
...
function resolver(partida) {
  ...
  // One shield, one hand: after a split with both hands lost, only the first one is covered. That is deliberate — the
  // shield is bought per use, so splitting shouldn't squeeze two refunds out of it.
