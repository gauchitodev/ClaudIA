...
// For shutdown (lib/cierre.js): refunds what's staked on every open hand, doubles and splits included. Not the
// insurance: it's settled the moment it's bought (the house peeks right then), so by now it was already lost.
// Returns { apuestas, monedas }.
export function devolverManosBlackjack() {
  let apuestas = 0;
  let monedas = 0;
  for (const [k, partida] of globalThis.manosBlackjack) {
    clearTimeout(partida.timeout);
    globalThis.manosBlackjack.delete(k);
    const enJuego = partida.manos.reduce((total, h) => total + h.cantidad, 0);
    ganarCoins(partida.chat, partida.usuario, enJuego, "casino_blackjack_devolucion");
    apuestas++;
    monedas += enJuego;
  }
