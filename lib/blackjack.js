...
function mirarCartaTapada(partida) {
  ...
  if (partida.seguro > 0) partida.seguroPerdido = true;
  return null;
}
...
export function repartir(chat, usuario, cantidad, alVencer, mazo = mazoNuevo()) {
  ...
  const partida = { chat, usuario, mazo, banca: [], manos: [{ cartas: [], cantidad, doblo: false, cerrada: false, dividida: false, deAses: false }], actual: 0, seguro: 0, seguroPerdido: false, ofreceSeguro: false, timeout: null, alVencer };
...
export function seguro(chat, usuario) {
  ...
  const cobro = cobrarApuesta(chat, usuario, costo, "casino_blackjack_seguro");
...
function resolver(partida) {
  ...
  if (partida.seguro > 0) {
    ...
    } else lineas.push(`🛡️ El seguro (${partida.seguro}) se perdió.`);
  }
  const apuestas = partida.manos.map((h) => h.cantidad).join(" + ");
  return {
    ok: true,
