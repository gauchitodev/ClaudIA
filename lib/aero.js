// Aeronautical weather: METAR and TAF from NOAA's Aviation Weather Center public API (no key needed).
// The METAR is decoded into Spanish using the fields the API already parses plus the raw text; the TAF is shown
// raw. There is a short per-station cache so NOAA isn't hit on every request from the group.
import { duracion } from "./tiempo.js";
import { textoSol } from "./aero-calculos.js";

export const FUENTE_NOAA = "_Fuente: NOAA, Aviation Weather Center._";
export const FUENTE_SOL = "_Fuente: ubicación de Open-Meteo; los horarios son cálculo propio._";
...
// Common names → ICAO, so you can write .metar carrasco or .metar punta. Matched without accents or case.
export const ALIAS = {
...
// "sumu punta xx" → { icaos: ["SUMU", "SULS"], invalidas: ["xx"] }. With no text, the default aerodrome.
...
// So the fetch can be injected in the tests.
export const _dep = { fetch: (...args) => fetch(...args) };
const cache = new Map();

// type "metar" → a list of objects (one per station with a report); "taf" → raw text (empty if there is none);
// "isigmet" → every live international SIGMET (it takes no filter, so the FIR filtering happens here).
...
// Visibility and phenomena come from the raw text, which in this region is in metres; the API gives them in miles.
...
export async function textoMetar(texto, ahora = Date.now()) {
  ...
  return [...bloques, ...avisos, FUENTE_NOAA].join("\n\n");
}
...
export async function textoSigmet(ahora = Date.now()) {
  ...
  return [cabecera, ...vigentes.map((s) => describirSigmet(s, ahora)), FUENTE_NOAA].join("\n\n");
}

// ---------- sun: city → coordinates via Open-Meteo, computed locally ----------
...
export function textoMenuAero() {
  ...
  return `✈️ *MENÚ AERO*
Meteorología aeronáutica y cálculos de cabina. Horas en UTC salvo que se aclare. Cada respuesta dice de dónde sale el dato.

*Observaciones y pronósticos*
▸ .metar — METAR de Carrasco, decodificado · NOAA
▸ .metar SULS — el de otro aeródromo (código ICAO), hasta ${AERO.MAX_ESTACIONES} por pedido · NOAA
▸ .metar punta salto — también por nombre · NOAA
▸ .taf — TAF (pronóstico) de Carrasco; .taf SULS para otro · NOAA
▸ .sigmet — avisos vigentes en la FIR Montevideo: tormentas, turbulencia, engelamiento, ceniza · NOAA
▸ .claro — claros horarios de las estaciones del país (viento, visibilidad, cielo, temperatura, presión) · Inumet
▸ .claro salto — el claro de una estación; .claro actualizar fuerza una lectura nueva · Inumet
...
▸ .sol — salida, puesta y crepúsculo civil en Montevideo; .sol Rivera para otra ciudad · ubicación de Open-Meteo, cálculo propio
▸ .zulu — hora Zulu, hora de Uruguay y las 25 zonas horarias con su letra militar
▸ .dtg — lo mismo en formato DTG (061532Z SEP 26)

*Fuentes*
METAR, TAF y SIGMET: NOAA (Aviation Weather Center). Claros: Inumet, leídos a los 10 de cada hora y guardados una hora. Sol: ubicación de Open-Meteo y cálculo propio. Lo demás es cálculo propio.
