...
import { generarPregunta, textoPregunta, abrirRonda, rondaDe } from "./trivia.js";
...
export function programarTriviasDelDia(ahora = new Date()) {
  ...
  for (const chat of chatsConOpcion("triviaRelampago")) {
    ...
    // No sale todos los días ni siempre la misma cantidad: cada una se tira a la suerte, y solo en grupos
    // que ayer tuvieron movimiento. Así se siente algo que pasa, no un reloj.
...
export async function lanzarTriviaRelampago(client, chat) {
  if (rondaDe(chat)) return;
  const pregunta = await generarPregunta(chat);
  const enviado = await client.sendMessage(chat, { text: textoPregunta({ titulo: "⚡ *TRIVIA RELÁMPAGO*", premio: COINS.TRIVIA_RELAMPAGO, pregunta, segundos: ACTIVIDAD.RELAMPAGO_SEGUNDOS }) });
  abrirRonda(chat, {
    tipo: "relampago",
    pregunta,
