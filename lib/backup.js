// Automatic database backup: one copy a day after 4:00 (the last 7 are kept) and, once a week, the day's copy is
// sent to the owner's private chat. It uses better-sqlite3's hot copy, which is consistent even while the bot is
// writing. The folder sits inside database/, which git already ignores.
import fs from "fs";
...
export async function hacerBackup() {
  ...
  fs.renameSync(temporal, destino); // this way a half-written copy never ends up under the final name
  rotar();
  return destino;
}
