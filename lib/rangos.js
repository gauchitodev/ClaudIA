...
export function chequearAscenso(chat, usuario, user, ahora = Date.now()) {
  if (!user) return null;
  ...
  if (rango.premio > 0) ganarCoins(chat, usuario, rango.premio, "rango_ascenso");
  const requisitos = `${rango.dias} ${rango.dias === 1 ? "día" : "días"} en el grupo y ${rango.mensajes} mensajes`;
  const premio = rango.premio > 0 ? ` +${rango.premio} UruCoins.` : "";
