...
// Rango del dueño del bot: no se gana con días ni mensajes, lo tiene quien esté en "owners" del config.toml.
// Va aparte de RANGOS para que no entre en la escalera automática ni dispare anuncios de ascenso.
...
export function rangoDe(chat, usuario, user, ahora = Date.now()) {
  ...
  // el owner tiene su propio rango y no sube más: no hay "siguiente"
...
export function chequearAscenso(chat, usuario, user, ahora = Date.now(), { pagar = true } = {}) {
  if (!user) return null;
  if (esOwner(usuario)) return null; // el owner no asciende: ya tiene su rango
