...
// ---------- Tragamonedas ----------
// Tres rodillos independientes. Con estos pesos y pagos la banca se queda con un 7 % a la larga.
const SIMBOLOS = [
...
export function jugarTragamonedas(chat, usuario, cantidad) {
  ...
  const emojis = rodillos.map((s) => s.emoji);
  let multiplicador = 0;
  let detalle = "";
  if (emojis.every((e) => e === emojis[0])) {
    multiplicador = rodillos[0].triple;
    detalle = `¡Tres ${emojis[0]}!`;
  } else if (emojis.filter((e) => e === "🍒").length === 2) {
    multiplicador = PAGA_DOS_CEREZAS;
    detalle = "Dos cerezas.";
  }
  const premio = cantidad * multiplicador;
  if (premio > 0) ganarCoins(chat, usuario, premio, "casino_tragamonedas_premio");
  const saldo = getSaldoCoins(chat, usuario);
  const resultado = premio > 0 ? `${detalle} Cobrás *${premio}* UruCoins (x${multiplicador}).` : "Nada esta vez.";
  return { ok: true, mensaje: `🎰 [ ${emojis.join(" | ")} ]\n${resultado} Te quedan ${saldo}.` };
}

export function textoPagosTragamonedas() {
  return SIMBOLOS.map((s) => `${s.emoji}${s.emoji}${s.emoji} x${s.triple}`).join(" · ") + ` · 🍒🍒 x${PAGA_DOS_CEREZAS}`;
}
