...
// ---------- Tragamonedas ----------
// Tres rodillos, tres filas visibles y cinco líneas de pago (filas y diagonales). Con estos pesos y pagos la banca se queda con un 7 % a la larga.
const SIMBOLOS = [
...
// Ventana de 3 rodillos x 3 filas con CINCO líneas de pago: las tres filas y las dos diagonales. La apuesta se
// reparte en partes iguales entre las cinco líneas y cada una paga por su cuenta:
// premio = apuesta x (suma de multiplicadores) / 5.
const LINEAS = [
...
export function jugarTragamonedas(chat, usuario, cantidad) {
  ...
  const resultados = LINEAS.map((l) => ({ nombre: l.nombre, ...evaluarLinea(l.celdas.map(([c, f]) => columnas[c][f])) }));
  const sumaMultiplicadores = resultados.reduce((s, r) => s + r.multiplicador, 0);
  const premio = Math.floor((cantidad * sumaMultiplicadores) / LINEAS.length);
  ...
  const filaGana = (f) => resultados[f].multiplicador > 0;
  const ventana = [0, 1, 2].map((f) => `${filaGana(f) ? "▶ " : "   "}${columnas.map((c) => c[f].emoji).join(" | ")}${filaGana(f) ? " ◀" : ""}`).join("\n");
  const ganadoras = resultados.filter((r) => r.multiplicador).map((r) => `${r.nombre}: ${r.detalle}`).join(" · ");
  const resultado = ganadoras ? `${ganadoras}\nCobrás *${premio}* UruCoins.` : devuelto ? `Ninguna línea pagó, pero tu escudo te devolvió los ${devuelto}.` : "Ninguna línea pagó esta vez.";
  return { ok: true, mensaje: `🎰 *TRAGAMONEDAS* — ${cantidad} UruCoins en ${LINEAS.length} líneas\n${ventana}\n\n${resultado} Te quedan ${saldo}.` };
}

export function textoPagosTragamonedas() {
  return `Por línea: ${SIMBOLOS.map((s) => `${s.emoji}${s.emoji}${s.emoji} x${s.triple}`).join(" · ")} · 🍒🍒 x${PAGA_DOS_CEREZAS}. La apuesta se reparte entre las 5 líneas: las tres filas y las dos diagonales.`;
}
