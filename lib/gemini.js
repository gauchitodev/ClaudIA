...
// Listening to an audio or searching the web takes longer than a plain chat reply.
const TIMEOUT_MS = 15000;
const TIMEOUT_LARGO_MS = 45000;

// The sources a web search used, from the response's groundingMetadata: [{ titulo, url }], no repeats.
export function fuentesDeBusqueda(candidato) {
  ...
  for (const chunk of candidato?.groundingMetadata?.groundingChunks || []) {
    ...
    vistas.add(url);
    fuentes.push({ titulo: chunk.web.title || url, url });
  }
  return fuentes;
}

// adjuntos: [{ mimeType, datos (base64) }], an audio for instance. buscarEnWeb: Google Search grounding, which can't go
// with a schema (Gemini silently drops the search when JSON output is asked for), so it's one or the other.
async function llamarModelo(modelo, texto, schema = null, { adjuntos = [], buscarEnWeb = false, clave = globalThis.geminiApiKey } = {}) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelo}:generateContent?key=${clave}`;
  const controller = new AbortController();
  const limite = adjuntos.length || buscarEnWeb ? TIMEOUT_LARGO_MS : TIMEOUT_MS;
  const timeoutId = setTimeout(() => controller.abort(), limite);
  try {
    const generationConfig = { temperature: 0.9, maxOutputTokens: 1500 };
    if (schema && !buscarEnWeb) {
      generationConfig.responseMimeType = "application/json";
      generationConfig.responseSchema = schema;
    }

    const parts = [...adjuntos.map((a) => ({ inline_data: { mime_type: a.mimeType, data: a.datos } })), { text: texto }];
    const body = {
    ...
    if (buscarEnWeb) body.tools = [{ google_search: {} }];
    ...
    // A grounded reply can come split in several parts; thoughts, if any, stay out.
    const respuesta = (candidato?.content?.parts || [])
      .filter((p) => typeof p.text === "string" && !p.thought)
      .map((p) => p.text)
      .join("");
    if (respuesta.trim()) return { ok: true, texto: respuesta.trim(), fuentes: fuentesDeBusqueda(candidato) };
    return { ok: false, sinCuota: false };
  } catch (error) {
    clearTimeout(timeoutId);
    if (error.name === "AbortError") {
      console.error(`[gemini] ${modelo} no respondió en ${limite / 1000}s, se corta y sigue con el siguiente`);
    } else {
...
// Every Gemini key there is (geminiApiKey plus geminiApiKeys in config.toml, see globals.js). Each key has its own free
// quota, so when one runs out the next one takes over. Just the one key when that's all there is.
export function clavesGemini() {
  const lista = globalThis.geminiApiKeys?.length ? globalThis.geminiApiKeys : [globalThis.geminiApiKey];
  return [...new Set(lista.filter(Boolean))];
}

// How a model with no quota shows up in .estado: with the key's number once there's more than one key.
export const etiquetaCuota = (modelo, indice, total) => (total > 1 ? `${modelo} (clave ${indice + 1})` : modelo);

export async function preguntarGemini(texto, opciones = {}) {
  const { schema = null, adjuntos = [], buscarEnWeb = false } = opciones;
  const claves = clavesGemini();
  if (!claves.length) return { ok: false, texto: null, sinCuota: false, modelo: null };

  let soloFallosPorCuota = true;

  // The best model first, on every key; only when all of them are out of quota comes the next model. So the extra keys
  // stretch the good model, instead of handing the chat to the Lite while another key still has quota for it.
  for (const modelo of MODELOS) {
    for (const [indice, clave] of claves.entries()) {
      const etiqueta = etiquetaCuota(modelo, indice, claves.length);
      const marcadoDesde = globalThis.modeloSinCuotaDesde.get(etiqueta);
      if (marcadoDesde && Date.now() - marcadoDesde < COOLDOWN_REINTENTO_MS) continue;

      const r = await llamarModelo(modelo, texto, schema, { adjuntos, buscarEnWeb, clave });
      if (r.ok) {
        globalThis.modeloSinCuotaDesde.delete(etiqueta);
        return { ok: true, texto: r.texto, fuentes: r.fuentes, sinCuota: false, modelo };
      }
      if (r.sinCuota) {
        // Searches have their own, smaller quota: running out of it mustn't take the model away from the chat.
        if (!buscarEnWeb) globalThis.modeloSinCuotaDesde.set(etiqueta, Date.now());
        console.log(`[gemini] ${etiqueta} sin cuota, probando el siguiente...`);
        continue;
      }
      // Any other failure (a 503, a timeout) is the model's, not the key's: one more try, then the next model.
      soloFallosPorCuota = false;
      const r2 = await llamarModelo(modelo, texto, schema, { adjuntos, buscarEnWeb, clave });
      if (r2.ok) {
        globalThis.modeloSinCuotaDesde.delete(etiqueta);
        return { ok: true, texto: r2.texto, fuentes: r2.fuentes, sinCuota: false, modelo };
      }
      break;
    }
  }
