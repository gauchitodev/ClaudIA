// Horse race with UruCoins: the first bet opens the group's race, for CARRERA_SEGUNDOS everyone bets (on different
// horses if they like), and on closing it runs: three "frames" of the track and the result with the payouts. Each
// race rolls the horses' strength, so the favourites and the odds shift; the odds already include the house margin
// (expected value ~0.93, like the rest of the casino). Bets follow the same casino caps.
...
// Rolls each horse's strength and works out its probability and odds (with the house margin).
...
// Records a bet; if no race is open it opens one. "alLargar({ cuadros, resultado })" is called on closing.
...
// Picks the winner by the probabilities, builds the three frames of the track and settles the bets.
export function correr(carrera) {
  ...
  for (const a of apuestas) {
    ...
    const devuelto = gano ? 0 : protegerApuesta(chat, a.usuario, a.cantidad, "escudo_carrera");
...
// For shutdown (lib/cierre.js): refunds every bet on a race that hasn't run. Returns { apuestas, monedas }.
export function devolverCarreras() {
  let apuestas = 0;
  let monedas = 0;
  for (const [chat, carrera] of globalThis.carreras) {
    clearTimeout(carrera.timeout);
    globalThis.carreras.delete(chat);
    for (const a of carrera.apuestas) {
      ganarCoins(chat, a.usuario, a.cantidad, "casino_carrera_devolucion");
      apuestas++;
      monedas += a.cantidad;
    }
  }
