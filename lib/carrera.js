...
function armarCaballos() {
  const fuerzas = CABALLOS.map(() => randomInt(10, 41));
...
export function correr(carrera) {
  ...
  const L = CARRERA.LARGO_PISTA;
  let r = randomInt(0, 1000000) / 1000000;
  ...
  const finales = caballos.map((c) => (c === ganador ? L : randomInt(Math.floor(L * 0.55), L)));
  const cuadro = (avance) => pista(caballos, finales.map((f) => Math.max(1, Math.min(L - 1, Math.round(f * avance) + randomInt(-1, 2)))));
  ...
  for (const a of apuestas) {
    ...
    lineas.push(`${gano ? "✅" : "❌"} ${mencion(a.usuario)} ${a.cantidad} a ${a.caballo.nombre}${gano ? ` → cobra ${premio}` : ""}`);
