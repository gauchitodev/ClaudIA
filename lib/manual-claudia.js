// Lo que Claudia sabe de sí misma: quién la hizo, qué comandos tiene, qué le pasó y qué le cambiaron.
//
// Qué se inyecta en el prompt y cuándo:
//   RESUMEN  → siempre (es corto).
//   MANUAL   → solo si el mensaje parece preguntar por comandos o funciones.
//   HISTORIA → solo si le preguntan qué le pasó, por el número nuevo, el baneo, etc.
//   CAMBIOS  → solo si le preguntan qué hay de nuevo, qué le cambiaron, la versión.
//
// MANTENIMIENTO: esto es texto escrito a mano, no se genera solo. Cuando agreguen un comando o
// hagan una actualización, súmenlo acá (a MANUAL y a CAMBIOS). Si no está acá, Claudia no lo sabe,
// y si le preguntan va a decir que no sabe en vez de inventar. Eso último es a propósito.
...
- .metar [ICAO o nombre, ej. SULS o punta]: METAR de un aeródromo decodificado en español (sin nada, Carrasco); .taf el pronóstico TAF; .sigmet los avisos vigentes de la FIR Montevideo (tormentas, turbulencia, engelamiento, ceniza); .cruzado 06 190 19 calcula viento cruzado y de frente para una pista con un viento dado; .sol [ciudad] la salida y puesta del sol y el crepúsculo civil; .zulu la hora Zulu, la de Uruguay y las 25 zonas horarias con su letra militar; .dtg lo mismo en formato DTG (061532Z SEP 26); .menuaero explica y lista los nombres que entiende. Datos de la NOAA, horas en UTC salvo .sol, que va en hora local del lugar.
...
// Lo que le pasó al bot. Contalo con naturalidad si preguntan, sin dramatizar y sin echarle la culpa a nadie.
...
// Registro de cambios. El más nuevo va PRIMERO. Escribilo en criollo, como se lo contarías al grupo.
