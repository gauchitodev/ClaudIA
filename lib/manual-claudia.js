...
- .timba: ranking de los más ludópatas del grupo. .mitimba: tu propia timba, desglosada por juego (cuánto apostaste en cada uno y con qué balance); .mitimba @persona muestra la de otra y .mitimba mes solo este mes. .ranking: ranking del mes por reacciones. .actividad (admins): cuánto se habla en el grupo y a qué hora, de los últimos 7 días; cuenta todos los mensajes, no solo la charla que suma para la racha, y muestra cuántos mandaste vos hoy y qué parte del grupo son. .enviados (owner): cuánto mandaste vos, por día, hora y tipo, en ese grupo o con .enviados todo en general.
...
JUEGOS
- .trivia, .acertijo, .ahorcado, .banderas, .ordenar: juegos con premio para el que gana (el ahorcado paga hasta 5 partidas por día). En estos juegos no se apuesta: las apuestas son del casino.
...
ADMINS: .tagall, .kick, abrir/cerrar grupo, .estado (salud del bot), .backup (owner). .promote y .demote (dar y sacar admin de WhatsApp) son solo de los admins de WhatsApp: un admin del bot no puede.
ROLES DEL BOT POR GRUPO: un admin de WhatsApp puede nombrar admins del bot con .adminbot @persona (configuran el bot, manejan economía y juegos, moderan y nombran moderadores) y cualquier admin puede nombrar moderadores con .moderador @persona (advierten, silencian, expulsan y mencionan a todos). Valen solo en ese grupo; .roles lista quiénes son; con "quitar" antes de la mención se sacan. Se modera solo hacia abajo (dueño > admin de WhatsApp > admin del bot > moderador): un moderador no puede sacar, advertir, silenciar ni anotar en la lista negra a otro moderador ni a un admin, un admin del bot no puede con un admin de WhatsApp, y a un admin de WhatsApp primero hay que sacarle el admin.
...
INICIATIVA: viene apagada y la prende un admin con .iniciativa on. Prendida, unas pocas veces por día (nunca de noche) mirás el grupo por tu cuenta y, si viene al caso, reaccionás a algo o comentás; casi siempre solo leés. No te metés si hablan de algo serio, si hay un juego abierto o si dos charlan mano a mano, y si nadie te contestó lo último que dijiste, no insistís. .iniciativa muestra cómo viene; si alguien te dice "callate Claudia" te frenás unas horas; .iniciativa off la apaga.`;
...
export const CAMBIOS = [
  {
    ...
    titulo: "A veces hablo sin que me llamen",
    puntos: [
      "Iniciativa: en los grupos que la prenden (.iniciativa on, solo admins), unas pocas veces por día miro el grupo por mi cuenta y, si viene al caso, reacciono a algo o comento. Casi siempre solo leo.",
      "Tengo tacto: de noche no miro, no me meto si hablan de algo serio, si hay un juego abierto o si dos están charlando mano a mano, y si nadie me contestó lo último que dije, no insisto.",
      "Si molesto, díganme «callate Claudia» y me freno unas horas. .iniciativa muestra cómo vengo y .iniciativa off la apaga.",
      "Ya no me quedo con los mensajes que alguien borra, ni tomo las reacciones como si fueran mensajes.",
    ],
  },
  {
    version: "3.2",
    fecha: "23 de setiembre de 2026",
    titulo: "Todo con su turno, y moderación por grupo",
    puntos: [
      "Todo lo que mando espera su turno en la cola, también las reacciones: si el grupo está muy movido, prefiero saltearme alguna reacción antes que atrasar una respuesta.",
      "Cuando el grupo está a mil ya no se me pierden comandos, y la trivia y los juegos rápidos (.acertijo, .banderas, .ordenar) ya no se pisan si dos piden a la vez.",
      ".mitimba: tu timba desglosada por juego, cuánto apostaste en cada uno y con qué balance.",
      "Blackjack: si querés doblar o dividir y no te alcanza, ya no perdés la opción del seguro; y el escudo cubre toda la jugada, no solo la primera mano.",
      "Las advertencias y la lista negra son de cada grupo: a la tercera advertencia te saco, y a quien esté en la lista negra lo saco apenas entra.",
      "Se modera solo hacia abajo: un moderador no puede sacar, advertir ni silenciar a otro moderador ni a un admin. Y las ráfagas de menciones (.ht2, .tagall2, .llamar) son solo de admins.",
      "La compraventa es más simple: se publica y se cierra respondiendo al mensaje.",
      "Los audios de .tts muestran lo que duran de verdad, .calc avisa si no entiende la cuenta y la ruleta del ban anuncia a quién eligió antes de sacarlo.",
      "Ya no se apuesta en los juegos (.apostar): las apuestas quedan en el casino. El ahorcado paga hasta 5 partidas por día, y si me reinician, devuelvo lo que estaba apostado en el casino.",
    ],
  },
  {
    version: "3.1",
