...
export const CACHE_GRUPOS = {
  ...
  ESPERA_MAX_MS: 3000, // how long a read waits for a forced refresh before settling for the copy at hand
};
...
// The promise's answer, or "siNoLlega" if it takes longer than ms.
function conTope(promesa, ms, siNoLlega) {
  let reloj;
  const tope = new Promise((resolve) => {
    reloj = setTimeout(() => resolve(siNoLlega), ms);
  });
...
// What to do with one entry of a "chats.update" event. Turning disappearing messages on or off in a group doesn't
// arrive as a group event: Baileys turns the notification into a protocol message (Socket/messages-recv.js) and reports
// it here, as ephemeralExpiration on the chat (Utils/process-message.js). The cached metadata carries
// ephemeralDuration, which Baileys reads to put an expiry on every message the bot sends to the group
// (Socket/messages-send.js): without this, for up to VIDA_MS they went out with the old setting. chats.update also
// fires for every ordinary message, which carries no ephemeralExpiration and is left alone.
export function aplicarCambioDeChat(client, cambio) {
  if (!esGrupo(cambio?.id) || !("ephemeralExpiration" in cambio)) return false;
  invalidar(client, cambio.id);
  return true;
}
...
export async function metadataDe(client, chat, { fresca = false, ahora = Date.now() } = {}) {
  ...
  //
  // But not for long. Baileys gives a query up to 60 s (defaultQueryTimeoutMs), and a slow answer held up every
  // message of the group, the bot's own sends included, with a copy from a minute ago at hand: past ESPERA_MAX_MS
  // that copy will do. A message sent in that window may leave out whoever just joined; their phone then asks for it
  // again, and Baileys resends it from its recent-message cache without going through the group list
  // (Socket/messages-recv.js, sendMessagesAgain). With no copy at all there's nothing better, so it waits.
  if (pendiente?.forzada) {
    const guardada = cache.get(chat);
    return guardada ? conTope(pendiente.promesa, CACHE_GRUPOS.ESPERA_MAX_MS, guardada.metadata) : pendiente.promesa;
  }
