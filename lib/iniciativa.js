...
// her? did someone tell her to be quiet?) and the .iniciativa status; the rules that decide whether she may speak are
// in lib/tacto.js, and the glance itself in lib/vistazos.js.
import { getChat, estadoIniciativa, guardarEstadoIniciativa, intervencionesDesde, ecoDeIntervenciones, marcarIntervencionRespondida, marcarUltimaPropiaRespondida, sumarReaccionAIntervencion } from "../database-functions.js";
...
export const INICIATIVA = {
  ...
  SILENCIO_CALLATE_MS: 3 * HORA, // "callate Claudia"
...
export const CALLATE = /\b(callate|call[ae]nse|shh+|silencio|basta|no molestes|no jodas|deja de joder|anda a dormir|nadie te (pregunto|llamo|hablo|dijo nada)|no te metas|cerra (el |la )?(pico|orto|culo|boca|jeta)|que (pesada|densa|intensa|hincha))\b/;
...
// her or quote her matter: a "callate" silences her for a while; otherwise, quoting one of her spontaneous texts, or
// naming her soon after one, counts as an answer to it. Returns "callate", "respondida" or null.
export function registrarMensajeDelGrupo(m, ahora = Date.now()) {
  ...
  if (CALLATE.test(texto)) {
    guardarEstadoIniciativa(m.chat, { silencioHasta: ahora + INICIATIVA.SILENCIO_CALLATE_MS, silencioMotivo: "le pidieron que se calle" });
    return "callate";
  }
...
export function textoIniciativa(chat, ahora = Date.now()) {
  ...
  lineas.push(`Topes por día: ${INICIATIVA.TOPE_TEXTOS_DIA} mensajes y ${INICIATIVA.TOPE_REACCIONES_DIA} reacciones. Si molesto: «callate Claudia». .iniciativa off la apaga.`);
