// A group's UruCoins economy dashboard: how much is in circulation, what came in and went out over a period grouped
// by category, and the highest balances. It all comes from urucoins_log, which records every movement.
...
const RUBROS = [
  ...
  ["duelo_", "Duelos"],
  ["sueldo_laburo", "Sueldos de laburos"],
  ["cambio_laburo", "Cambios de laburo"],
...
export function textoEconomia(chat, dias = 7) {
  ...
  if (e.ricos.length) lineas.push("", "*Saldos más altos:*", ...e.ricos.map((r, i) => `${i + 1}. ${nombreDe(r.usuario)} — ${r.saldo}`));
  // a quick read for the admin: if far more comes in than goes out, the coins inflate
  ...
  return { texto: lineas.join("\n"), mentions: [] }; // names without tagging: it is a dashboard, not a notice
}
