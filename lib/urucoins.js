...
// One entry per game: "<chat>|chat|<nombre>" for whole-group games, "<chat>|<jugador>|<nombre>" for individual ones.
// By name because several group games can run at once in a chat (a riddle and a flags quiz, say): with a single
// "<chat>|chat" entry the second one wiped the first one's bets, and whichever ended first settled the other's.
const juegosActivos = new Map();
const clave = (chat, jugador, nombre) => `${chat}|${jugador || "chat"}|${nombre}`;

export function juegoIniciado(chat, nombre, jugador = null) {
  juegosActivos.set(clave(chat, jugador, nombre), { chat, nombre, jugador, apuestas: new Map() });
}

// The key of the chat's only active game of that kind (group, or that player's), for a call without a name. With
// several there's no telling which one ended: none is closed, and it's logged.
function unicaClave(chat, jugador) {
  const prefijo = `${chat}|${jugador || "chat"}|`;
  const claves = [...juegosActivos.keys()].filter((k) => k.startsWith(prefijo));
  if (claves.length > 1) console.error(`[urucoins] juegoTerminado sin nombre, con ${claves.length} juegos activos en ${chat}: no se cierra ninguno`);
  return claves.length === 1 ? claves[0] : null;
}

// Closes the game, pays out bets and hands the winner their prize. "opciones" is { nombre, jugador }, as given to
// juegoIniciado; a bare string is taken as the player, the way it was called before games had names. Returns a line
// to append to the game's final message ("" when there is nothing to say).
export function juegoTerminado(chat, ganador, opciones = {}) {
  const { jugador = null, nombre = null } = typeof opciones === "string" ? { jugador: opciones } : opciones || {};
  const k = nombre ? clave(chat, jugador, nombre) : unicaClave(chat, jugador);
  const juego = k ? juegosActivos.get(k) : null;
  if (k) juegosActivos.delete(k);
  ...
  if (juego && juego.apuestas.size > 0) {
    ...
    for (const [usuario, cantidad] of juego.apuestas) {
      ...
      } else if (protegerApuesta(chat, usuario, cantidad, "escudo_apuesta")) {
        escudos++;
...
// The active games a person can bet on in a chat: the group's and their own individual ones. "nombre", if given,
// keeps the one it names ("banderas", "bandera" or "Banderas").
function juegosParaApostar(chat, usuario, nombre = null) {
  const t = nombre ? String(nombre).toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "") : null;
  return [...juegosActivos.values()].filter((j) => j.chat === chat && (!j.jugador || j.jugador === usuario) && (!t || j.nombre === t || j.nombre.startsWith(t) || t.startsWith(j.nombre)));
}

// Bets on an active game: the group's, or the person's own individual one. With several active, "nombre" says which.
export function apostar(chat, usuario, cantidad, nombre = null) {
  ...
  const candidatos = juegosParaApostar(chat, usuario, nombre);
  if (candidatos.length === 0) {
    return { ok: false, error: nombre ? `No hay ningún juego de ${nombre} activo para apostar.` : "No hay ningún juego activo para apostar. Arrancá uno primero (trivia, acertijo, ahorcado...)." };
  }
  if (candidatos.length > 1) {
    const nombres = candidatos.map((j) => j.nombre);
    return { ok: false, error: `Hay varios juegos activos (${nombres.join(", ")}): decí a cuál apostás, por ejemplo .apostar ${cantidad} ${nombres[0]}` };
  }
  const [juego] = candidatos;
...
// For shutdown (lib/cierre.js): refunds every bet on a game still in play, and forgets the games. Returns
// { apuestas, monedas }.
export function devolverApuestasDeJuegos() {
  let apuestas = 0;
  let monedas = 0;
  for (const [k, juego] of juegosActivos) {
    juegosActivos.delete(k);
    for (const [usuario, cantidad] of juego.apuestas) {
      ganarCoins(juego.chat, usuario, cantidad, "apuesta_devolucion");
      apuestas++;
      monedas += cantidad;
    }
  }
