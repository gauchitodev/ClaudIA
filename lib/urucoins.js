...
export function otorgarPorReaccion(chat, autorLid, reactorLid, messageId) {
  ...
  if (dentroDelTope(autorLid, ganaAutor, multAutor)) ganarCoins(chat, autorLid, ganaAutor, "reaccion_recibida");

  const ganaReactor = COINS.REACCION_EMITIDA * multReactor;
  if (dentroDelTope(reactorLid, ganaReactor, multReactor)) ganarCoins(chat, reactorLid, ganaReactor, "reaccion_emitida");
