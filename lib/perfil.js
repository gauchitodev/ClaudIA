// A person's record in the group: it gathers into one message what the economy, the jobs, the activity, the couples,
// the birthdays and the shop each store. Used by .perfil.
...
import { monedasActivas, apuestaMaxima, COINS } from "./urucoins.js";
import { resumenReputacion } from "./reputacion.js";
import { parejaDe } from "./parejas.js";
import { resumenFamilia } from "./familia.js";
import { contarPublicacionesActivas } from "../database-functions.js";
import { etiquetaRol } from "./roles.js";

const mencion = (id) => `@${id.split("@")[0]}`;

// Returns { texto, mentions }. "user" is that person's users row (getUser); esPropio changes the title and the tone.
export function textoPerfil(chat, lid, user, esPropio = false) {
  ...
  // with the economy off (.monedas / .modo compraventa) the record shows no coins, job, streak, duels or items
  const conEconomia = monedasActivas(chat);
  if (conEconomia) {
    const saldo = getSaldoCoins(chat, lid);
    const puesto = puestoCoins(chat, lid);
    lineas.push(`🪙 ${saldo} UruCoins${puesto ? ` · puesto ${puesto} del grupo` : ""}${saldo >= COINS.APUESTA_MIN ? ` · apuesta máxima ${apuestaMaxima(chat, lid, COINS.CASINO_APUESTA_MAX)}` : ""}`);
  ...
  // duels played = bets minus refunds (challenges declined or expired and draws don't count as played)
  ...
  const familia = resumenFamilia(lid);
  if (familia) lineas.push(familia);
  ...
  // the inventory already carries the job line; here it goes at the top, so it's filtered out
