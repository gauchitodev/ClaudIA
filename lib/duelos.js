...
export function desafiar(chat, retador, retado, cantidad, tipoTexto, alVencer) {
  ...
  const peleando = tipo === "pelea" ? enOtraPelea(chat, [retador, retado]) : null;
  if (peleando) return { ok: false, error: peleando === retador ? "Terminá tu pelea antes de empezar otra." : `${mencion(retado)} está en medio de una pelea; esperá a que termine.`, mentions: [retado] };
...
export function aceptar(chat, retado, avisar = null) {
  ...
  // Checked again here: either of them may have started another fight since the challenge.
  const peleando = d.tipo === "pelea" ? enOtraPelea(chat, [d.retador, retado]) : null;
  if (peleando) return { ok: false, error: `${peleando === retado ? "Estás" : `${mencion(peleando)} está`} en otra pelea: esta arranca cuando termine. El desafío sigue en pie hasta que venza.`, mentions: [peleando] };
...
function jugar(d) {
  ...
  const devuelto = protegerApuesta(d.chat, perdedor, d.cantidad, "escudo_duelo");
...
// Who among them is already fighting in this chat, or null. One fight per person: a second one took over their key in
// "peleas", and the first, finding nobody on that turn, hung forever with both stakes inside.
function enOtraPelea(chat, usuarios) {
  return usuarios.find((u) => globalThis.peleas.has(clave(chat, u))) || null;
}
...
function terminarPelea(pelea, ganador, motivo) {
  ...
  const devuelto = perdedor ? protegerApuesta(pelea.chat, perdedor, pelea.cantidad, "escudo_duelo") : 0;
...
// For shutdown (lib/cierre.js): refunds the challenger of every pending challenge, and both people in every fight
// under way. Returns { apuestas, monedas }.
export function devolverDuelosYPeleas() {
  let apuestas = 0;
  let monedas = 0;
  for (const [k, d] of globalThis.duelos) {
    clearTimeout(d.timeout);
    globalThis.duelos.delete(k);
    ganarCoins(d.chat, d.retador, d.cantidad, "duelo_devolucion");
    apuestas++;
    monedas += d.cantidad;
  }
  for (const pelea of new Set(globalThis.peleas.values())) {
    clearTimeout(pelea.timeout);
    for (const u of pelea.orden) {
      globalThis.peleas.delete(clave(pelea.chat, u));
      ganarCoins(pelea.chat, u, pelea.cantidad, "duelo_devolucion");
      apuestas++;
      monedas += pelea.cantidad;
    }
  }
