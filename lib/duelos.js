// Duelos uno contra uno con UruCoins: alguien desafía a otra persona por una cantidad, la otra acepta, y se juega
// un dado (gana el número más alto, empate se repite), una carta (gana la más alta; el palo desempata) o una PELEA
// por turnos (cada uno elige golpe, patada, cubrirse o curar hasta que alguien se queda sin vida). Cada uno pone lo
// mismo y el ganador se lleva todo: la banca no toca nada. El desafío vence a los pocos minutos y devuelve lo
// apostado. Vive en memoria, como las mesas.
...
if (!globalThis.duelos) globalThis.duelos = new Map(); // "chat|retado" -> desafío pendiente
...
const PALOS = ["♣", "♦", "♥", "♠"]; // de menor a mayor, para desempatar
...
export function desafiar(chat, retador, retado, cantidad, tipoTexto, alVencer) {
  ...
  if (cantidad > COINS.DUELO_APUESTA_MAX) return { ok: false, error: `La apuesta máxima en un duelo es ${COINS.DUELO_APUESTA_MAX} UruCoins.` };
...
// "avisar(mensaje)" lo usa la pelea para mandar el turno automático cuando alguien no responde a tiempo.
...
function jugar(d) {
  ...
  if (d.tipo === "dado") {
    ...
    if (c2 >= c1) c2++; // dos cartas distintas del mismo mazo
  ...
  return { ok: true, ganador, texto: `${detalle}\n🏆 Gana ${mencion(ganador)} y se lleva *${pozo} UruCoins*.`, mentions: [d.retador, d.retado] };
}
...
// ====================== Pelea por turnos ======================
// Cada uno arranca con PELEA.HP de vida. En su turno elige: golpe (casi siempre conecta, daño medio), patada (más daño,
// pero falla más), cubrirse (el próximo golpe que reciba hace la mitad y puede contraatacar) o curar (recupera vida,
// CURAS veces por pelea). Si no elige en SEGUNDOS_TURNO, tira un golpe solo. A los MAX_TURNOS gana quien tenga más
// vida; empate devuelve lo apostado.
export const PELEA = { HP: 100, SEGUNDOS_TURNO: 45, MAX_TURNOS: 20, CURAS: 2 };
if (!globalThis.peleas) globalThis.peleas = new Map(); // "chat|usuario" -> pelea (las dos personas apuntan a la misma)
...
function terminarPelea(pelea, ganador, motivo) {
  ...
  return `${motivo}\n🏆 ${mencion(ganador)} gana la pelea y se lleva *${pozo} UruCoins*.`;
}

// Acción del jugador en su turno. Devuelve { ok, texto, mentions, terminada } o { ok: false, error }.
