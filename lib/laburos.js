...
export const LABURO = {
  ...
  DINAMICO_PENALIZACION_MAX: 0.4, // uno saturado paga como mínimo −40 %
};

export const OFICIOS = {
  tambero: { nombre: "Tambero", emoji: "🐄", sueldo: 10, desc: "Te levantás a las 5. Las vacas no saben de feriados.",
  ...
  camionero: { nombre: "Camionero", emoji: "🚛", sueldo: 12, desc: "Ruta, termo y radio. Cobra bien, dormís poco.",
  ...
  peon: { nombre: "Peón de estancia", emoji: "🐎", sueldo: 9, desc: "Alambrar, aparte, esquila. Sueldo justo, cuero curtido.",
  ...
  guardavidas: { nombre: "Guardavidas", emoji: "🏖️", sueldo: 8, desc: "Silla alta, silbato y protector. En invierno cobrás igual, no preguntes.",
  ...
  chofer: { nombre: "Chofer de ómnibus", emoji: "🚌", sueldo: 10, desc: "Línea del interior, paradas que no existen en el mapa.",
  ...
  oficinista: { nombre: "Oficinista", emoji: "💼", sueldo: 9, desc: "Planillas, mails y aire acondicionado. Estable, sin gloria.",
  ...
  dj: { nombre: "DJ", emoji: "🎧", sueldo: 11, desc: "Fiestas, cumples de 15 y algún boliche. Cobrás cuando hay evento.",
  ...
  politico: { nombre: "Político", emoji: "🏛️", sueldo: 6, desc: "Paga poco, todos te odian, y encima te cae la prensa.",
...
export function etiquetaLaburo(chat, usuario) {
  ...
  return l ? `${l.oficio.emoji} ${l.oficio.nombre}` : "";
}
...
export function tomarLaburo(chat, usuario, clave) {
  ...
  const costo = actual ? ` Pagaste ${LABURO.CAMBIO_COSTO} de trámites.` : "";
...
export function cobrar(chat, usuario) {
  ...
  const conteo = afiliadosPorOficio();
  const cobrado = Math.max(1, Math.round(sueldoActual(actual.laburo, conteo) * mult * rachaMult));
  ...
  return { ok: true, mensaje: `${actual.oficio.emoji} ${texto}\n🪙 Cobraste *${cobrado} UruCoins*${racha}. Tenés ${getSaldoCoins(chat, usuario)}.${dinamico}` };
}
...
export function textoLaburos(chat, usuario) {
  ...
  const pie = actual ? `Ahora sos ${actual.oficio.nombre.toLowerCase()}. Cambiar cuesta ${LABURO.CAMBIO_COSTO} y hay ${LABURO.CAMBIO_ESPERA_DIAS} días de espera.` : "Elegí uno con .laburo <nombre>. El primero es gratis.";
  const topes = `+${Math.round(LABURO.DINAMICO_BONUS_MAX * 100)} % / −${Math.round(LABURO.DINAMICO_PENALIZACION_MAX * 100)} %`;
  return `💼 *LABUROS*\n\n${lineas.join("\n\n")}\n\n${pie} Cobrás una vez por día con .cobrar.\nEl sueldo sube en los oficios con poca gente y baja en los llenos (hasta ${topes}), contando a la gente de todos los grupos del bot.`;
}
