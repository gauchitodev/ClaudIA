...
// sube o baja. El oficio aparece en .coins y .timba, y Claudia lo sabe para chicanear en la charla.
...
export function cobrar(chat, usuario) {
  ...
  const cobrado = Math.max(1, Math.round(actual.oficio.sueldo * mult * rachaMult));
  ...
  return { ok: true, mensaje: `${actual.oficio.emoji} ${texto}\n🪙 Cobraste *${cobrado} UruCoins*${racha}. Tenés ${getSaldoCoins(chat, usuario)}.` };
}

// ---------- textos ----------
export function textoLaburos(chat, usuario) {
  const actual = laburoDe(chat, usuario);
  const lineas = Object.entries(OFICIOS).map(([clave, o]) => `${o.emoji} *${o.nombre}* — ${o.sueldo}/día · .laburo ${clave}\n${o.desc}`);
