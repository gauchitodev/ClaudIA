...
export function renunciar(chat, usuario) {
  ...
  // The row stays, with no trade and no paydays, so the last payday's date survives. Deleting it let anyone collect,
  // quit, take another job (free, as a first one) and collect again the same day, as many times as they liked.
  tabla().prepare(`UPDATE laburos SET laburo = '', cobros = 0 WHERE chat = ? AND usuario = ?`).run(chat, usuario);
...
export function topLaburantes(chat, n = 5) {
  return tabla().prepare(`SELECT usuario, laburo, cobros FROM laburos WHERE chat = ? AND laburo != '' ORDER BY cobros DESC LIMIT ?`).all(chat, n); // '' = quit
}
