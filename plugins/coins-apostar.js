...
plugin.run = async (m, { client, args }) => {
  ...
  if (!args[0] || Number.isNaN(cantidad)) {
    return client.sendText(m.chat, `¿Cuánto apostás? Ej: .apostar 20 (mínimo ${COINS.APUESTA_MIN}). Tiene que haber un juego activo; si hay varios, decí a cuál: .apostar 20 trivia.`, m);
  }

  const resultado = apostar(m.chat, m.sender, cantidad, args[1]); // args[1]: which game, when several are on
