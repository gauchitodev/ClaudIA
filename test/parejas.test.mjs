...
test("parejas: pedir, aceptar y terminar; un pedido pendiente no es una pareja", () => {
  ...
  // while it's pending, 2 can take a kiss from anyone and 1 can keep asking someone else
  ...
  // they get back together: the ex stops showing as an ex while they're together
...
test("parejas: rechazar, vencimiento y aceptar cuando alguno se puso de novio mientras tanto", () => {
  ...
  P.aceptarPareja("3@lid", "1@lid"); // 3 started dating 1 before 4 answered
...
test("parejas: casamiento con una semana de relación, propuesta, sí y no, y ayudas del owner", () => {
  ...
  // the owner forms couples outright, and if either was with someone else that relationship ends
  ...
  // hand-typed "@number"s require at least three digits, as in lidMencionado
...
test("besar: un pedido de pareja pendiente no bloquea el beso, una pareja de verdad sí", async () => {
  ...
  const besa = async (de, a) => {
    ...
    return globalThis.enviados.find((x) => x.msg?.text)?.msg.text; // the last send may be the reaction
  };
  P.pedirPareja("1@lid", "2@lid"); // 1 asked 2, no answer
  ...
  // the attempt with 3 left 2 in a huff: a gift fixes it, and with no neglect in play a kiss between partners is never turned down
  ...
  assert.equal(globalThis.enviados.at(-1).msg.react.text, "💍");
  // with no partner, it's sometimes turned down
  P.terminarPareja("1@lid");
...
test("parejas: migración desde las columnas viejas de users", async () => {
  // 1 and 2 mutual and married, 3 asked 4 with no answer, 4 has 1 in their history
  ...
  globalThis.db = F.loadDatabase(); // idempotent: it neither duplicates nor overwrites
  assert.equal(P.listaParejas().length, 1);
});
...
test("parejas: enojo por infidelidad, reconciliación con regalo y rechazo por abandono", async () => {
  ...
  // in a huff again, and a gift fixes it
  ...
  // neglect: three days with no kiss or gift, the first kiss may be turned down once
  const luego = t0 + 4 * DIA_MS;
  P._dep.azar = () => 0; // the rejection comes up
  ...
  P._dep.azar = () => 0.9; // not this time
  assert.equal(P.besoEntrePareja("1@lid", "2@lid", luego + 5 * DIA_MS).tipo, "ok");
  assert.equal(P.parejaDe("1@lid").ultimoBeso, luego + 5 * DIA_MS);

  // the plugin: infidelity leaves the partner in a huff, the kiss is turned down with the reason, the gift makes up
  globalThis.txt = (await import("../lib/strings.js")).default;
  const { default: besar } = await import("../plugins/fun-besar.js");
